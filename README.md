...
services:
  frontend:
    build:
      context: "https://github.com/bailang1999-a11y/TG-Marketing-Assistant.git#${APP_GIT_REF:-v1.0.12}"
  ...
  gateway:
    build:
      context: "https://github.com/bailang1999-a11y/TG-Marketing-Assistant.git#${APP_GIT_REF:-v1.0.12}:backend"
      args:
        APP_VERSION: "${APP_VERSION:-1.0.12}"
    ...
    environment:
      ...
      APP_UPDATE_COMMAND: "cd /workspace && LATEST_TAG=$$(wget -qO- https://api.github.com/repos/bailang1999-a11y/TG-Marketing-Assistant/tags | sed -n 's/.*\"name\": \"\\(v[0-9][^\"]*\\)\".*/\\1/p' | head -n 1); export APP_GIT_REF=$${LATEST_TAG:-v1.0.12}; export APP_VERSION=$${APP_GIT_REF#v}; docker compose up -d --build --force-recreate --remove-orphans"
    depends_on:
      postgres:
        condition: service_healthy
      redis:
